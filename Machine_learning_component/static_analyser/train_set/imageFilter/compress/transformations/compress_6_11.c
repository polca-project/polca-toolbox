...
main()
{
 ...
 for(int i=0;i<SAMPLES;i++){
  ...
  /*   for(n = 0; n < 16; n++) { */
		  ...
		  block[t1][t2] = image[((B*(z/16)+t1)*128)+(B*(z%16)+t2)];
    		}
    		factor2 += factor1;
    	  }
...
//    	  int k;
      ...
      result[(z/16)][(z%16)][0] = pack4in32(block[0][0],block[0][1],
      				  pack2in8(block[0][2] / 3, block[0][3] / 3),
      				  pack2in8(block[0][4] / 2, block[0][5] / 2));

      result[(z/16)][(z%16)][1] = pack4in32(pack2in8(block[0][6],block[0][7]),
				  block[1][0],block[1][1],
				  pack2in8(block[1][2] / 2,block[1][3]));

      result[(z/16)][(z%16)][2] = pack4in32(pack2in8(block[1][4], block[1][5]),
				  pack2in8(block[2][0] / 3, block[2][1] / 2),
				  pack2in8(block[2][2] / 2, block[2][3]),
				  pack2in8(block[3][0] / 3, block[3][1]));

      result[(z/16)][(z%16)][3] = pack4in32(pack2in8(block[3][2], block[3][3]),
