    ...
    for (w = 0; w < width; w++) 
    {
      ...
      for (l = 0; l < filterLen; l++) 
      {
        ...
        for (k = 0; k < filterLen; k++) 
        {
          ...
          sum = sum + pImage[y * width + x] * pFilter[l * filterLen + k];
        }
      }
...
int main(int argc, char* argv[])
{
 ...
 for(int sample=0;sample<SAMPLES;sample++){
  ...
  /* First iteration peeled out from the main loop involving */
  /* - Convolution2D for all scales for octave=0 */
  /* - DoG for octave=0 */
    octave = 0;
    ...
    for (scale = 0; scale < params.noOfScales; scale++) 
    {
      ...
      /* Printing */
      sprintf(outFileName, "../data/gauss_%d_%d_out.pgm", octave, scale);
    ...
    for (scale = 0; scale < params.noOfScales - 1; scale++) 
    {
      ...
      /* Printing */
      sprintf(outFileName, "../data/dog_%d_%d_out.pgm", octave, scale);
  ...
  /* Rest of iterations for octaves > 1 */
  /* Loop over number of octaves */
  for (octave = 1; octave < params.noOfOctaves; octave++) 
  {
	...
	pImage = gauss[octave][0];
	/* Printing */
	sprintf(outFileName, "../data/gauss_%d_%d_out.pgm", octave, 0);
	saveImage(outFileName, width, height, params.fracLen, gauss[octave][0]);
    ...
    /* Construct Gaussian images */ 
      for (scale = 1; scale < params.noOfScales; scale++) 
    {
      /* Perform 2D convolution to obtain a gaussian image */
      convolution2D(width,
          height,
