...
int main(const int argc, const char** argv) {
 ...
 for(int sample=0;sample<SAMPLES;sample++){
  ...
  for (int i = 0; i < nBodies*DIM; i+=DIM) {
	...
	F[0] = k==0 && j==0 ? 0.0f : F[0];
	F[1] = k==0 && j==0 ? 0.0f : F[1];
	F[2] = k==0 && j==0 ? 0.0f : F[2];

	dx = k==0 ? pos[j*3]   - pos[i] : dx;
	dy = k==0 ? pos[j*3+1] - pos[i+1] : dy;
	dz = k==0 ? pos[j*3+2] - pos[i+2] : dz;
	...
	F[0] = (k==0 ? dx * invDist3 : 0.0f) + F[0]; 
	F[1] = (k==0 ? dy * invDist3 : 0.0f) + F[1]; 
	F[2] = (k==0 ? dz * invDist3 : 0.0f) + F[2];
