 ...
 for(int sample=0;sample<SAMPLES;sample++){
  ...
  for (int iter = 1; iter <= nIters; iter++) {
    ...
    for (int ijkw = 0; ijkw < nBodies*nBodies*3*3; ijkw++) {
      ...
      // System eqs.
      /* int nj = ijkw-(i*54+j*6+k*2); */
      /* int ni = ijkw-(i*51+j*9+k*2); */
      // symbolic
      int nj = ijkw - (((nBodies * DIM * DIM)) * i +((DIM * DIM) - 3) * j +(DIM - 1) * k);
      int ni = ijkw - (((nBodies * DIM * DIM) - 3) * i +((DIM * DIM)) * j +(DIM - 1) * k);

      d[ijkw] = w==0 ? pos[nj]   - pos[ni] : d[ijkw];
      
      // hand coded
      /* distSqr[ijkw]  = w==0 && k==2 ? d[ijkw-3*(2-0)]*d[ijkw-3*(2-0)] + d[ijkw-3*(2-1)]*d[ijkw-3*(2-1)] + d[ijkw-3*(2-2)]*d[ijkw-3*(2-2)] + SOFTENING : distSqr[ijkw]; */
      // System eqs.
      /* distSqr[ijkw]  = w==0 && k==2 ? d[ijkw-6]*d[ijkw-6] + d[ijkw-3]*d[ijkw-3] + d[ijkw-0]*d[ijkw-0] + SOFTENING : distSqr[ijkw]; */
      // symbolic
      distSqr[ijkw]  = w==0 && k==2 ? d[ijkw-((DIM-1)*DIM)]*d[ijkw-((DIM-1)*DIM)] + d[ijkw-(((DIM-1)-1)*DIM)]*d[ijkw-(((DIM-1)-1)*DIM)] + d[ijkw-(((DIM-1)-2)*DIM)]*d[ijkw-(((DIM-1)-2)*DIM)] + SOFTENING : distSqr[ijkw];

      invDist[ijkw]  = w==0 && k==2 ? 1.0f / sqrtf(distSqr[ijkw]) : invDist[ijkw];
      ...
      // symbolic
      /* F[ijkw]    = k==2 ? (j==0 ? 0.0f : F[ijkw-(DIM*DIM)]) + d[ijkw-(((DIM-1)-w)*DIM+w)] * invDist3[ijkw-w] : F[ijkw]; */
      F[ijkw]    = k==2 ? (j==0 ? 0.0f : F[ijkw-(DIM*DIM)]) + d[ijkw-(-(0*DIM - 0 - (DIM-1)*DIM + (DIM-1))/(0 - (DIM-1)) * w +(0*(DIM-1)*DIM + 0*(DIM-1) - (DIM-1)*(DIM-1)*DIM)/(0 - (DIM-1)))] * invDist3[ijkw-w] : F[ijkw];

      F[ijkw]    = k==2 ? (j==0 ? 0.0f : F[ijkw-(DIM*DIM)]) + d[ijkw-(-(DIM*0 - DIM*(DIM-1) - 1*0 + 1*(DIM-1))/(0 - (DIM-1)) * w +(DIM*0*(DIM-1) - DIM*(DIM-1)*(DIM-1) + 1*0*(DIM-1))/(0 - (DIM-1))
)] * invDist3[ijkw-w] : F[ijkw];
