...
int main(const int argc, const char** argv) {
 ...
 for(int sample=0;sample<SAMPLES;sample++){
  ...
  for (int iter = 1; iter <= nIters; iter++) {
    ...
    int j,k;
    int t,i;
    for (int t1 = 0; t1 < nBodies*nBodies*3; t1++) {
      i = t1 / (nBodies*3);
      t = t1 % (nBodies*3);
      j = t/3;
      k = t%3;
      ...
      Fx = (k==2 ? d[0] * invDist3 : 0.0f) + (t==0 ? 0.0f : Fx);
      Fy = (k==2 ? d[1] * invDist3 : 0.0f) + (t==0 ? 0.0f : Fy);
      Fz = (k==2 ? d[2] * invDist3 : 0.0f) + (t==0 ? 0.0f : Fz);

      if(k==2) printf("[%2d,%2d,%2d] -> % f % f % f | % f % f % f\n",i,j,k,d[0],d[1],d[2],Fx,Fy,Fz);

      vel[i*DIM]   = t==(nBodies*3)-1 ? vel[i*DIM] + dt*Fx   : vel[i*DIM];
      vel[i*DIM+1] = t==(nBodies*3)-1 ? vel[i*DIM+1] + dt*Fy : vel[i*DIM+1];
      vel[i*DIM+2] = t==(nBodies*3)-1 ? vel[i*DIM+2] + dt*Fz : vel[i*DIM+2];
