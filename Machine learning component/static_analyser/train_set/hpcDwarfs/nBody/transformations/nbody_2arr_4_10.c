...
int main(const int argc, const char** argv) {
 ...
 for(int sample=0;sample<SAMPLES;sample++){
  ...
  for (int i = 0; i < nBodies*DIM; i+=DIM) {
    ...
    for (int j = 0; j < nBodies*DIM; j+=DIM) {
      float dx;
      float dy;
      float dz;
      float distSqr;
      ...
      float d[1];
#pragma roll-up XXXX = pos[j+YYYY] - pos[i+ZZZZ]
      for(int k=0;k<1;k++){
	d[k] = pos[j+k]   - pos[i+k];
      }
      dy = pos[j+1] - pos[i+1];
      dz = pos[j+2] - pos[i+2];
      distSqr = d[0]*d[0] + dy*dy + dz*dz + SOFTENING;
      invDist = 1.0f / sqrtf(distSqr);
      invDist3 = invDist * invDist * invDist;

      Fx += d[0] * invDist3; Fy += dy * invDist3; Fz += dz * invDist3;
    }
    vel[i] += dt*Fx; vel[i+1] += dt*Fy; vel[i+2] += dt*Fz;
  }
