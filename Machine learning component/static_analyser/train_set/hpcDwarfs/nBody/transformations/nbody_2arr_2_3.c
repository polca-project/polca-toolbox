...
int main(const int argc, const char** argv) {
 ...
 for(int sample=0;sample<SAMPLES;sample++){
  ...
  for (int i = 0; i < nBodies*DIM; i+=DIM) {
    ...
    for(int p=0;p<P;p++) {
      int prev_chunk_size = nBodies / P;
      int curr_chunk_size = p != (P-1) ? prev_chunk_size : prev_chunk_size + nBodies%P;
      for (int j = p*prev_chunk_size;j<((p+1)*curr_chunk_size); j++) {
	float dx = pos[j*DIM]   - pos[i];
	float dy = pos[j*DIM+1] - pos[i+1];
	float dz = pos[j*DIM+2] - pos[i+2];
	float distSqr = dx*dx + dy*dy + dz*dz + SOFTENING;
	float invDist = 1.0f / sqrtf(distSqr);
	float invDist3 = invDist * invDist * invDist;
