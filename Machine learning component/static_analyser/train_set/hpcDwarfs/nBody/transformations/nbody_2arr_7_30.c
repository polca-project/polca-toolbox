 ...
 for(int sample=0;sample<SAMPLES;sample++){
  ...
  for (int i = 0; i < nBodies*DIM; i+=DIM) {
    ...
    for (int j = 0; j < nBodies; j++) {
      ...
      for(int k=0;k<3;k++){
	d[k] = pos[j*3+k]   - pos[i+k];
      }
      distSqr = d[0]*d[0] + d[1]*d[1] + d[2]*d[2] + SOFTENING;
      invDist = 1.0f / sqrtf(distSqr);
      invDist3 = invDist * invDist * invDist;

      Fx = (j==0 ? 0.0f : Fx) + d[0] * invDist3;
      Fy = (j==0 ? 0.0f : Fy) + d[1] * invDist3;
      Fz = (j==0 ? 0.0f : Fz) + d[2] * invDist3;

      vel[i]   = j==nBodies-1 ? vel[i]   + dt*Fx : vel[i];
      vel[i+1] = j==nBodies-1 ? vel[i+1] + dt*Fy : vel[i+1];
      vel[i+2] = j==nBodies-1 ? vel[i+2] + dt*Fz : vel[i+2];
