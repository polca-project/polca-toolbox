...
//    	  int k;
      ...
      result[(z/16)][(z%16)][0] = pack4in32(block[0*B+0],block[0*B+1],
      				  pack2in8(block[0*B+2] / 3, block[0*B+3] / 3),
      				  pack2in8(block[0*B+4] / 2, block[0*B+5] / 2));

      result[(z/16)][(z%16)][1] = pack4in32(pack2in8(block[0*B+6],block[0*B+7]),
				  block[1*B+0],block[1*B+1],
				  pack2in8(block[1*B+2] / 2,block[1*B+3]));

      result[(z/16)][(z%16)][2] = pack4in32(pack2in8(block[1*B+4], block[1*B+5]),
				  pack2in8(block[2*B+0] / 3, block[2*B+1] / 2),
				  pack2in8(block[2*B+2] / 2, block[2*B+3]),
				  pack2in8(block[3*B+0] / 3, block[3*B+1]));

      result[(z/16)][(z%16)][3] = pack4in32(pack2in8(block[3*B+2], block[3*B+3]),
