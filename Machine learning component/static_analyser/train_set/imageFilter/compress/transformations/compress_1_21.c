...
//      int k;
      ...
      for (i = 0; i < B; i++) {
        for (j = 0; j < B; j++) {
          sum = 0.0;
          for (k = 0; k < B; k++) {
            sum += image[((B*(z/16)+i)*128)+(B*(z%16)+k)] * cos1[j*B+k];
          }
          temp2d[i*B+j] = sum;
      ...
      /* Multiply the cosine matrix by the temporary matrix; store the
         result back in the original matrix.  */
          ...
          /* round the result */
          image[((B*(z/16)+i)*128)+(B*(z%16)+j)] = ROUND(sum);
        }
...
/////////////////////////////////////////////////
  ...
  for (z = 0; z < 16 * 16; z++) {
      ...
      result[(z/16)][(z%16)][0] = pack4in32(image[((B*(z/16)+0)*128)+(B*(z%16)+0)],image[((B*(z/16)+0)*128)+(B*(z%16)+1)],
				  pack2in8(image[((B*(z/16)+0)*128)+(B*(z%16)+2)] / 3, image[((B*(z/16)+0)*128)+(B*(z%16)+3)] / 3),
				  pack2in8(image[((B*(z/16)+0)*128)+(B*(z%16)+4)] / 2, image[((B*(z/16)+0)*128)+(B*(z%16)+5)] / 2));

      result[(z/16)][(z%16)][1] = pack4in32(pack2in8(image[((B*(z/16)+0)*128)+(B*(z%16)+6)],image[((B*(z/16)+0)*128)+(B*(z%16)+7)]),
				  image[((B*(z/16)+1)*128)+(B*(z%16)+0)],image[((B*(z/16)+1)*128)+(B*(z%16)+1)],
				  pack2in8(image[((B*(z/16)+1)*128)+(B*(z%16)+2)] / 2,image[((B*(z/16)+1)*128)+(B*(z%16)+3)]));

      result[(z/16)][(z%16)][2] = pack4in32(pack2in8(image[((B*(z/16)+1)*128)+(B*(z%16)+4)], image[((B*(z/16)+1)*128)+(B*(z%16)+5)]),
				  pack2in8(image[((B*(z/16)+2)*128)+(B*(z%16)+0)] / 3, image[((B*(z/16)+2)*128)+(B*(z%16)+1)] / 2),
				  pack2in8(image[((B*(z/16)+2)*128)+(B*(z%16)+2)] / 2, image[((B*(z/16)+2)*128)+(B*(z%16)+3)]),
				  pack2in8(image[((B*(z/16)+3)*128)+(B*(z%16)+0)] / 3, image[((B*(z/16)+3)*128)+(B*(z%16)+1)]));

      result[(z/16)][(z%16)][3] = pack4in32(pack2in8(image[((B*(z/16)+3)*128)+(B*(z%16)+2)], image[((B*(z/16)+3)*128)+(B*(z%16)+3)]),
				  pack2in8(image[((B*(z/16)+4)*128)+(B*(z%16)+0)] / 2, image[((B*(z/16)+4)*128)+(B*(z%16)+1)]),
				  pack2in8(image[((B*(z/16)+5)*128)+(B*(z%16)+0)] / 2, image[((B*(z/16)+5)*128)+(B*(z%16)+1)]),
				  pack2in8(image[((B*(z/16)+6)*128)+(B*(z%16)+0)] , image[((B*(z/16)+7)*128)+(B*(z%16)+0)]));
